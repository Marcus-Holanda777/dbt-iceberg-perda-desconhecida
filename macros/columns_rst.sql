{% macro set_tipos() %}
{% set tipos = ["rx", "salao", "psico_termo", "entrada_direta", "grt", "marca_propria"] %}
{{ return(tipos) }}
{% endmacro %}
...
{% macro columns_perc(column_value, flag = true) %}
    ...
    {% for tp in tipos -%}
    {%- if tp == 'grt' and not flag -%}{% continue %}{%- endif -%}
    {{ column_value }}_{{ tp }} / NULLIF({{ column_value }}, 0) as perc_{{ suff }}_{{ tp }}
    {%- if not loop.last -%}
        ,
    {% endif %}
{%- endfor -%}
...
{% macro columns_ind(column_inv, column_venda, flag = false) %}
    ...
    {% for tp in tipos -%}
    {%- if tp == 'grt' and not flag -%}{% continue %}{%- endif -%}
    {{ column_inv }}_{{ tp }} / NULLIF({{ column_venda }}, 0) as ind_{{ suff }}_{{ tp }}
    {%- if not loop.last -%}
        ,
    {% endif %}
{%- endfor -%}
