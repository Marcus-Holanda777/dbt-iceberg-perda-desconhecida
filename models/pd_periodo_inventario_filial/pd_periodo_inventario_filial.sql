with pre_resumo as
(
	select * from {{ ref('perdas') }} full outer join {{ ref('vendas') }} using(filial)
), pos_resumo as
(
select
    filial,
...
select 
    *,
    CASE
	    WHEN ind_inv * 100 > 0.70 THEN 'Extrema'
		WHEN ind_inv * 100 > 0.40 THEN 'Alta'
		WHEN ind_inv * 100 > 0.30 THEN 'Media'
		WHEN ind_inv * 100 < 0.30 THEN 'Baixa'
	END                                                                                          AS criticidade,
    valor_inv     / nullif(date_diff('day', penultimo_inventario, ultimo_inventario), 0)         AS perda_diaria_inv,
    valor_inv_ant / nullif(date_diff('day', ante_penultimo_inventario, penultimo_inventario), 0) AS perda_diaria_inv_ant
from pos_resumo
)
select
	 rst.filial
...
from resumo as rst
inner join {{ source('prevencao-perdas', 'excel_base_supervisores') }}   as fil on rst.filial = fil.filial
left join {{ source('prevencao-perdas', 'excel_base_seguranca_atual') }} as atl on rst.filial = atl.filial